...
public class CartController {
    ...
    public ApiResponse<Void> add(@Valid @RequestBody CartAddRequest request) {
        cartService.add(AuthContext.getRequiredUserId(), request);
        return ApiResponse.success("加入购物车成功", null);
    }

    @GetMapping("/list")
    public ApiResponse<List<CartItemVO>> list() {
        return ApiResponse.success(cartService.listCart(AuthContext.getRequiredUserId()));
    }

    @PutMapping("/update")
    public ApiResponse<Void> update(@Valid @RequestBody CartUpdateRequest request) {
        cartService.updateCart(AuthContext.getRequiredUserId(), request);
        return ApiResponse.success("更新成功", null);
    }

    @DeleteMapping("/{id}")
    public ApiResponse<Void> delete(@PathVariable Long id) {
        cartService.deleteCart(AuthContext.getRequiredUserId(), id);
